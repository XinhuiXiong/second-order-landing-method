    ...
    def run(
        ...
        *,
        ...
        while True:
            ...
            # Add log
            ortho_error  = float(np.linalg.norm(x.T @ x - np.eye(x.shape[1]), ord="fro"))
            if self._log_verbosity >= 1:
                elapsed = float(time.time() - start_time)
                self._add_log_entry(iteration=iteration, point=x, cost=fx, tangent_residual=norm_grad, time=elapsed, ortho_error= ortho_error)
